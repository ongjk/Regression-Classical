...
```{r}
flightsB <-  flightsA %>%
  filter(month == 12, origin %in% c('LGA', 'ATL')) %>%
  select(name, day, sched_dep_time, dep_delay, arr_delay, air_time, distance) %>%
...
```{r}
flightsC <- flightsB %>%
  mutate(loss = arr_delay - dep_delay, loss_ratio = loss/dep_delay, avg_speed = (distance / air_time) * 60)
...
```{r}
flightsD <- flightsC[complete.cases(flightsC), ] %>%
...
Summarise the number of flights, the average departure delay, and the average arrival delay by carrier and arrange by average arrival delay. Save the results as `flightsE`. Run `glimpse()` and to show the structure then print out the contents of `flightsE`.

```{r}
flightsE <- flightsD %>%
  group_by(carrier_name) %>%
  summarize(n_flight = n(), avg_depdelay = mean(dep_delay), avg_arrdelay = mean(arr_delay)) %>%
...
ANSWER: -inf, Nan look problematic. Division of small number or zero. 
...
ANSWER: NaN, division of zero or empty set. 

```{r}
flightsD[!complete.cases(flightsD), ]
```
