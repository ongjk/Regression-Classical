...
```{r}
ggplot(anthroX, aes(x = height)) +
  geom_histogram()
ggplot(anthroX, aes(x = gender, y = height)) +
  geom_boxplot()
```


ANSWER: I don't see any particular value that are too unusual looking at the histogram and the boxplot. I just removed all the NA and reduced the gender to male or female. I didn't want to mess with the numbers too much. Ideal looks like its going to break some pattern with a bigger range but I figure its what people want and I don't want to recalculate someone's fantasy. 
...
ggplot(anthroX, aes(x = height, y = ideal)) +
  ...
  geom_smooth(method = "lm", aes(x = height, y = ideal), se = FALSE, linetype = 9, color = "red") +
...
anthropred_data <- tibble(height  = c(63, 64, 69, 70, 72, 74),
...
ANSWER: Looking at the lm model men would tend to be happier if they are taller and get further if they are shorter. The opposite is shown for the women, where they are closer to the ideal if they are shorter but get further if they are taller. 
