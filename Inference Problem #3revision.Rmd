...
In your simulations, you need to compare proportions of numbers <= 195 (for 1970) and <= 125 (for 1971), like in your descriptives, not the means of the draft numbers. Otherwise the sims are generally correctly executed. You do not need to do t-tests, since we're comparing proportions. Why did you decide there was unfairness? Support your decision.
...
observed_diff <- mean(draftnumber_S <= 195) - mean(draftnumber_F <= 195)
...
for (i in 1:N)
{
  index <- sample(n_all, size = n_S, replace = FALSE) 
  random_diffs[i] <- mean(draftnumberG[index]  <= 195 ) - mean(draftnumberG[-index] <= 195)
}
...
observed_diff <- mean(draftnumber_S <= 125) - mean(draftnumber_F <= 125)
...
{
  index <- sample(n_all, size = n_S, replace = FALSE) 
  random_diffs[i] <- mean(draftnumberG[index] <= 125) - mean(draftnumberG[-index] <= 125)
}
...
**ANSWER:**  A: Yes, There is unfairness for the draft in 1970 but not for 1971. The resampling distribution shows whether or not it was due to randomness. Looking at the histograms and p-value for 1970 and 1971 shows that the observed difference in 1970 is unusual while the one in 1971 isn't. 
